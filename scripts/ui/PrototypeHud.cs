...
public partial class PrototypeHud : CanvasLayer
{
    ...
    public override void _Ready()
    {
        ...
        _director = GetTree().GetFirstNodeInGroup("game_director") as GameDirector;
    ...
    {
        if (_staminaLabel is not null && _player is not null)
        {
            _staminaLabel.Text = $"Stamina: {(int)_player.CurrentStamina}/{(int)_player.MaxStamina}";
        }

        if (_debugLabel is not null && _player is not null)
        {
            Vector2 input = _player.LastMovementInput;
            Vector2 position = _player.GlobalPosition;
            _debugLabel.Text = $"Input: {input.X:0.0}, {input.Y:0.0}  Pos: {(int)position.X}, {(int)position.Y}";
        }
